export default function GroupCard({groupName, members}) {
  const formatMembers = (members) => {
    ...
    if(members.length === 2) return `${members[0].User.username} and ${members[1].User.username}`
    if(members.length === 3) return `${members[0].User.username}, ${members[1].User.username}, and ${members[2].User.username}`
    if(members.length > 3) return `${members[0].User.username}, ${members[1].User.username}, and ${members.length - 2} others`
  }
